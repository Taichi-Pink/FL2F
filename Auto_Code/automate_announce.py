...
for i in range(0, total_rows):
    ...
    if(l_index<6):
        if(sheet.iloc[i]["Activity"]==lecture_name[l_index]):
            ...
            show_video = compare_with_today_in_range(today, event_date, 1)
            if (l_index < 5):
                if(show_video):
                    video_lenth = 23
                    for x in range(video_nums[l_index]):
                        replace_one_file_line(dir_lec + lecture_file[l_index], video_lenth, "  hide_from_lectures: false")  # not hide in lecture section
                        video_lenth = video_lenth + 3
                else:
                    video_lenth = 23
                    for x in range(video_nums[l_index]):
                        replace_one_file_line(dir_lec + lecture_file[l_index], video_lenth, "  hide_from_lectures: true")
                        video_lenth = video_lenth + 3
            l_index = l_index + 1
